...
let
  ...
  initUser =
    ...
    ,
      ...
      extraGroups = unique (groups ++ (if isAdmin then adminGroups else [ ]));
    };
  ...
  mkNixOSUser = attrs:
    ...
    in
    {
      ...
      if (length user.allowedKeys) == 0 then { }
      else { openssh.authorizedKeys.keys = user.allowedKeys; }
    ) // (
      if user.initialPassword == "" then { }
      else { inherit (user) initialPassword; }
    );

  # Build a NixOS module to configure the user
  mkNixOSUserModule = users:
    let
      cfg = listToAttrs (map
        (user: {
          inherit (user) name;
...
in
{
  user = { inherit initUser mkNixOSUser mkNixOSUserModule mkNixOSHMModule mkHMUserModule getHomeConfig; };
}
